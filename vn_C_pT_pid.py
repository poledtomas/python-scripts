...
def def_ave_4particle_correlation(pxn, pyn, qx2n, qy2n, Qxn, Qyn, Qx2n, Qy2n, M, mp, mq):
    qxn = pxn
    qyn = pyn

    Qn_sq = Qxn * Qxn + Qyn * Qyn

    one = pxn * Qxn * Qxn * Qxn + pxn * Qxn * Qyn * Qyn + pyn * Qxn * Qxn * Qyn + pyn * Qyn * Qyn * Qyn
...
def four_particle_correlation(Qxn, Qyn, Qx2n, Qy2n, M):
    ...
    term1=abs(Qn*Qn*Qn.conjugate()*Qn.conjugate())+abs(Qn2*Qn2.conjugate())-2*(Qn2*Qn.conjugate()*Qn.conjugate()).real
            ...
            for iev in range(0, NoE, eventStep):
                ...
                for _ in range(eventStep):
                    ...
                    if npart > 0:
                        ...
                        for _ in range(npart):
                            ...
                            if (ele != 0 and abs(eta) < etaCut and pt > ptMinCut and pt < ptMaxCut and id == pid):
                                RFP += 1
                ...
                if RFP>0:
                    ...
                    for ipt in range (ptBins):
                        ...
                        if (POI[ipt] > 0):
                            commulant[ipt]=def_ave_2partucle_correlation(qx[ipt],qy[ipt],Qx,Qy,POI[ipt],POI[ipt],RFP)
                            cumulantfour[ipt]=def_ave_4particle_correlation(qx[ipt],qy[ipt],qx2[ipt],qy2[ipt], Qx, Qy, Qx2,Qy2, RFP,POI[ipt],POI[ipt])
...
vn_C_pT_pid(str(sys.argv[1]),str(sys.argv[2]),int(sys.argv[3]),int(sys.argv[4]),int(sys.argv[5]),int(sys.argv[6]))
